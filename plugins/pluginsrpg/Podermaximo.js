...
const TEXTOS_PODER_MAX = [
  "📈 {nombre} forzó el límite de *{personaje}* y ganó 💳 {creditos} créditos y ✨ {xp} XP.",
  "🧪 {nombre} llevó a *{personaje}* a su máximo y obtuvo 💳 {creditos} créditos y ✨ {xp} XP.",
  "🔬 {nombre} empujó el potencial de *{personaje}* y recibió 💳 {creditos} créditos y ✨ {xp} XP.",
  "🛠️ {nombre} optimizó el poder máximo de *{personaje}* y consiguió 💳 {creditos} créditos y ✨ {xp} XP.",
  "🚀 {nombre} desató el modo máximo de *{personaje}* y ganó 💳 {creditos} créditos y ✨ {xp} XP."
];
...
const handler = async (msg, { conn }) => {
  ...
  if (usuario.ultimoPoderMax && (ahora - usuario.ultimoPoderMax) < COOLDOWN_MS) {
    ...
    return conn.sendMessage(chatId, { text: `⏳ Debes esperar *${m}m ${s}s* para volver a usar *podermaximo*.` }, { quoted: msg });
  }
  ...
  if (restanteCred <= 0 && restanteXP <= 0) {
    return conn.sendMessage(chatId, { text: `🛑 Límite diario de *podermaximo* alcanzado.\nVuelve mañana.` }, { quoted: msg });
  }
  ...
  if (cred <= 0 && xp <= 0) {
    return conn.sendMessage(chatId, { text: `🛑 Ya alcanzaste el tope diario de *podermaximo*.` }, { quoted: msg });
  }
