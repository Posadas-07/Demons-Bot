...
const TEXTOS_TALAR = [
  "🪵 {nombre} taló árboles antiguos y obtuvo 💳 {creditos} créditos y ✨ {xp} XP.",
  "🪚 {nombre} hizo leña de calidad, ganando 💳 {creditos} créditos y ✨ {xp} XP.",
  "🌲 {nombre} seleccionó maderas finas y consiguió 💳 {creditos} créditos y ✨ {xp} XP.",
  "🪓 {nombre} partió troncos perfectos y ganó 💳 {creditos} créditos y ✨ {xp} XP.",
  "🌳 {nombre} limpió el bosque y recibió 💳 {creditos} créditos y ✨ {xp} XP.",
  "📦 {nombre} vendió tablones pulidos por 💳 {creditos} créditos y ✨ {xp} XP.",
  "🚚 {nombre} entregó madera premium y obtuvo 💳 {creditos} créditos y ✨ {xp} XP."
];
...
const handler = async (msg, { conn }) => {
  ...
  if (usuario.ultimoTalar && (ahora - usuario.ultimoTalar) < COOLDOWN_MS) {
    ...
    return conn.sendMessage(chatId, { text: `⏳ Debes esperar *${min}m ${seg}s* para volver a talar.` }, { quoted: msg });
  }
  ...
  if (restanteCred === 0 && restanteXP === 0) {
    return conn.sendMessage(chatId, { text: `🛑 Límite diario de *talar* alcanzado. Vuelve mañana. 😊` }, { quoted: msg });
  }
  ...
  if (creditosOtorgados === 0 && xpOtorgada === 0) {
    return conn.sendMessage(chatId, { text: `🛑 Ya alcanzaste el tope diario de *talar*.` }, { quoted: msg });
  }
