...
// Textos aleatorios
const TEXTOS_DARAGUA = [
  "💧 {nombre} dio agua fresca a *{mascota}* y obtuvo 💳 {creditos} créditos y ✨ {xp} XP.",
  "🚰 {nombre} hidrató a *{mascota}* y ganó 💳 {creditos} créditos y ✨ {xp} XP.",
  "🥤 {nombre} sirvió bebida energizante a *{mascota}* y recibió 💳 {creditos} créditos y ✨ {xp} XP.",
  "🌊 {nombre} llevó a *{mascota}* a la fuente y consiguió 💳 {creditos} créditos y ✨ {xp} XP.",
  "🧊 {nombre} ofreció agua fría a *{mascota}* y obtuvo 💳 {creditos} créditos y ✨ {xp} XP.",
  "🪣 {nombre} llenó el bebedero de *{mascota}* y ganó 💳 {creditos} créditos y ✨ {xp} XP.",
  "🚿 {nombre} refrescó a *{mascota}* y recibió 💳 {creditos} créditos y ✨ {xp} XP."
];
...
const handler = async (msg, { conn }) => {
  ...
  if (usuario.ultimoDarAgua && (ahora - usuario.ultimoDarAgua) < COOLDOWN_MS) {
    const falta = Math.ceil((COOLDOWN_MS - (ahora - usuario.ultimoDarAgua)) / 1000);
    const min = Math.floor(falta / 60);
    const seg = falta % 60;
    return conn.sendMessage(
      chatId,
      { text: `⏳ Debes esperar *${min}m ${seg}s* para volver a dar agua.` },
      { quoted: msg }
    );
  }
  ...
  if (!usuario.daraguaDiario || usuario.daraguaDiario.fecha !== hoy) {
    usuario.daraguaDiario = { fecha: hoy, creditos: 0, xp: 0 };
  }
  const restanteCred = Math.max(0, TOPE_CREDITOS_DIA - (usuario.daraguaDiario.creditos || 0));
  const restanteXP = Math.max(0, TOPE_XP_DIA - (usuario.daraguaDiario.xp || 0));

  if (restanteCred === 0 && restanteXP === 0) {
    return conn.sendMessage(
      chatId,
      { text: `🛑 Límite diario de *daragua* alcanzado.\nHoy ya farmeaste *${TOPE_CREDITOS_DIA} créditos* y *${TOPE_XP_DIA} XP* con este comando.` },
      { quoted: msg }
    );
  }
  ...
  if (creditosOtorgados === 0 && xpOtorgada === 0) {
    return conn.sendMessage(chatId, { text: `🛑 Ya alcanzaste el tope diario de *daragua*.`, quoted: msg });
  }

  // Aplicar recompensas
  usuario.ultimoDarAgua = ahora;
  ...
  usuario.daraguaDiario.creditos += creditosOtorgados;
  usuario.daraguaDiario.xp += xpOtorgada;

  // Guardar DB
  fs.writeFileSync(sukirpgPath, JSON.stringify(db, null, 2));

  // Mensaje FINAL (solo lo ganado + notificaciones de subida) — citando al usuario
  const nombreTexto = `${usuario.nombre} ${usuario.apellido}`.trim();
  const mascotaNombre = String(mascota.nombre || "tu mascota");
  const base = TEXTOS_DARAGUA[Math.floor(Math.random() * TEXTOS_DARAGUA.length)]
