...
const TEXTOS_ENEMIGOS = [
  "⚔️ {nombre} enfrentó a poderosos enemigos con *{personaje}* y ganó 💳 {creditos} créditos y ✨ {xp} XP.",
  "🛡️ {nombre} defendió la aldea de invasores junto a *{personaje}* y obtuvo 💳 {creditos} créditos y ✨ {xp} XP.",
  "💥 {nombre} venció a un grupo de villanos con *{personaje}* y recibió 💳 {creditos} créditos y ✨ {xp} XP.",
  "🏹 {nombre} cazó enemigos peligrosos con *{personaje}* y consiguió 💳 {creditos} créditos y ✨ {xp} XP.",
  "🗡️ {nombre} se enfrentó a una banda de criminales con *{personaje}* y ganó 💳 {creditos} créditos y ✨ {xp} XP."
];
...
const handler = async (msg, { conn }) => {
  ...
  if (usuario.ultimoEnemigos && (ahora - usuario.ultimoEnemigos) < COOLDOWN_MS) {
    const falta = Math.ceil((COOLDOWN_MS - (ahora - usuario.ultimoEnemigos)) / 1000);
    const min = Math.floor(falta / 60);
    const seg = falta % 60;
    return conn.sendMessage(chatId, { text: `⏳ Debes esperar *${min}m ${seg}s* para volver a luchar contra enemigos.` }, { quoted: msg });
  }

  // Límite diario
  const hoy = hoyStrLocal();
  usuario.enemigosDiario = usuario.enemigosDiario || { fecha: hoy, creditos: 0, xp: 0 };
  if (usuario.enemigosDiario.fecha !== hoy) {
    usuario.enemigosDiario = { fecha: hoy, creditos: 0, xp: 0 };
  }
  const restanteCred = Math.max(0, TOPE_CREDITOS_DIA - usuario.enemigosDiario.creditos);
  const restanteXP = Math.max(0, TOPE_XP_DIA - usuario.enemigosDiario.xp);
  if (restanteCred <= 0 && restanteXP <= 0) {
    return conn.sendMessage(chatId, { text: `🛑 Límite diario de *enemigos* alcanzado.\nHoy ya farmeaste *${TOPE_CREDITOS_DIA} créditos* y *${TOPE_XP_DIA} XP* con este comando.` }, { quoted: msg });
  }
  ...
  if (creditosGanados <= 0 && xpGanada <= 0) {
    return conn.sendMessage(chatId, { text: `🛑 Ya alcanzaste el tope diario de *enemigos*.` }, { quoted: msg });
  }

  usuario.ultimoEnemigos = ahora;
  ...
  usuario.enemigosDiario.creditos += creditosGanados;
  usuario.enemigosDiario.xp += xpGanada;

  fs.writeFileSync(filePath, JSON.stringify(db, null, 2));

  const base = TEXTOS_ENEMIGOS[Math.floor(Math.random() * TEXTOS_ENEMIGOS.length)]
