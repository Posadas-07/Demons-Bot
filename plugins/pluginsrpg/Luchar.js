...
const TEXTOS_LUCHAR = [
  "⚔️ {nombre} luchó contra enemigos y ganó 💳 {creditos} créditos y ✨ {xp} XP para *{personaje}*.",
  "🛡️ {nombre} defendió su reino y obtuvo 💳 {creditos} créditos y ✨ {xp} XP para *{personaje}*.",
  "🏹 {nombre} participó en una batalla épica y recibió 💳 {creditos} créditos y ✨ {xp} XP para *{personaje}*.",
  "🗡️ {nombre} derrotó a un jefe y consiguió 💳 {creditos} créditos y ✨ {xp} XP para *{personaje}*.",
  "🔥 {nombre} peleó valientemente y ganó 💳 {creditos} créditos y ✨ {xp} XP para *{personaje}*."
];
...
const handler = async (msg, { conn }) => {
  ...
  if (usuario.ultimoLuchar && (ahora - usuario.ultimoLuchar) < COOLDOWN_MS) {
    ...
    return conn.sendMessage(chatId, { text: `⏳ Debes esperar *${min}m ${seg}s* para volver a luchar.` }, { quoted: msg });
  }
  ...
  if (creditosGanados <= 0 && xpGanada <= 0) {
    return conn.sendMessage(chatId, { text: `🛑 Ya alcanzaste el tope diario de *luchar*.` }, { quoted: msg });
  }
