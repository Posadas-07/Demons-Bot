...
const TEXTOS_OTROUNIVERSO = [
  "🌠 {nombre} cruzó el multiverso con *{personaje}* y ganó 💳 {creditos} créditos y ✨ {xp} XP.",
  "🌌 {nombre} visitó un universo paralelo con *{personaje}* y obtuvo 💳 {creditos} créditos y ✨ {xp} XP.",
  "🪐 {nombre} exploró mundos alternos con *{personaje}* y recibió 💳 {creditos} créditos y ✨ {xp} XP.",
  "🚀 {nombre} viajó por realidades infinitas con *{personaje}* y consiguió 💳 {creditos} créditos y ✨ {xp} XP.",
  "🔮 {nombre} descubrió dimensiones ocultas con *{personaje}* y ganó 💳 {creditos} créditos y ✨ {xp} XP."
];
...
const handler = async (msg, { conn }) => {
  ...
  if (usuario.ultimoOtrouniverso && (ahora - usuario.ultimoOtrouniverso) < COOLDOWN_MS) {
    ...
    const seg = falta % 60;
    return conn.sendMessage(chatId, { text: `⏳ Debes esperar *${min}m ${seg}s* para volver a viajar a otro universo.` }, { quoted: msg });
  }
  ...
  if (restanteCred <= 0 && restanteXP <= 0) {
    return conn.sendMessage(chatId, { text: "🛑 Has alcanzado el límite diario de *otrouniverso* (9 000 créditos y 10 000 XP). Inténtalo mañana." }, { quoted: msg });
  }
  ...
  if (creditosGanados <= 0 && xpGanada <= 0) {
    return conn.sendMessage(chatId, { text: "🛑 Con esta acción superarías el límite diario de *otrouniverso*. Vuelve mañana.", quoted: msg }, { quoted: msg });
  }
